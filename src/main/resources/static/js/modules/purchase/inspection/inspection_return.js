...
function addEntry() {			// 로컬스토리지 저장
    ...
    // 🔹 저장 제한 체크
    if (!savelimitCheck("barcodeListInspectionReturn", 500)) {
    ...
    if ((barcode.split("_").length === 6)){
        ...
        if (window.localStorage && localStorage.getItem("barcodeListInspectionReturn")) {
            stored = JSON.parse(localStorage.getItem("barcodeListInspectionReturn"));
        } else {
            stored = [];
        }
        ...
        if (stored.includes(barcode)) {
            ...
            stored.push(barcode);
            localStorage.setItem("barcodeListInspectionReturn", JSON.stringify(stored));
            $("#barcodeInput").val("");
            renderTable();
            hideLoading();
...
function saveBarcode(kind) {				// 반품검사 전송 (kind: 'RETURN' | 'SCRAP')
    if (isSaving) return;
    isSaving = true;

    const barcodeList = JSON.parse(localStorage.getItem("barcodeListInspectionReturn") || "[]");
    ...
    Utils.showConfirm(confirmMsg, () => {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (response === "success") {
                        localStorage.removeItem('barcodeListInspectionReturn');
                        $("#dataTableBody").empty();
                        $("#count").text("0");
                        $("#totalqty").text("0");
...
function renderTable() {		//테이블그리기
    ...
    if (typeof localStorage !== 'undefined' && localStorage.getItem("barcodeListInspectionReturn")) {
        barcodeArray = JSON.parse(localStorage.getItem("barcodeListInspectionReturn"));
    } else {
        barcodeArray = [];
    }
    table.empty();
...
function deleteEntry(bar, qty) {		// localstorage에서 특정데이터 삭제
    ...
    Utils.showConfirm(m("confirm.delete.item"), () => {
        let barcodeArray = JSON.parse(localStorage.getItem("barcodeListInspectionReturn") || "[]");
        let newArray = barcodeArray.filter(item => item.toString().trim() !== bar.toString().trim());
        localStorage.setItem("barcodeListInspectionReturn", JSON.stringify(newArray));
