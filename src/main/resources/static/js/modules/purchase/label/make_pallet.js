...
$(document).ready(function () {
    ...
    $("#allSendBtn").off("click").on("click", saveBarcode);
...
function addEntry() {			// 로컬스토리지 저장
    ...
    // 🔹 저장 제한 체크
    if (!savelimitCheck("barcodeListPallet", 500)) {
    ...
    if ((barcode.split(",").length === 5 && (barcode.split(",")[4] === "SCMMEX" || barcode.split(",")[4] === "WMSMEX") || barcode.split(",")[4] === "WMSUSA")) {
        let stored = [];
        let saveItemcode = "";
        if (window.localStorage && localStorage.getItem("barcodeListPallet")) {
            stored = JSON.parse(localStorage.getItem("barcodeListPallet"));
            if (stored.length > 0 && typeof stored[0] === 'string') {
                saveItemcode = stored[0].split(",")[0]
        ...
        if (stored.includes(barcode)) {
            ...
            stored.push(barcode);
            localStorage.setItem("barcodeListPallet", JSON.stringify(stored));
            $("#barcodeInput").val("");
            renderTable();
            hideLoading();
        ...
        Utils.showAlert(barcode, warning);
        let audio = new Audio('/sounds/buzzer.wav');
        audio.play();
        $("#barcodeInput").val("");
        hideLoading();
...
function saveBarcode() {
    ...
    isSaving = true;
    console.log("isaving true : " + isSaving);
    // 버튼 비활성화
    $("#allSendBtn").prop("disabled", true);

    const barcodeList = JSON.parse(localStorage.getItem("barcodeListPallet") || "[]");
    ...
    if (barcodeList.length === 0) {
        ...
        // ❗ return하기 전에 반드시 되돌려야 함
        isSaving = false;
        $("#allSendBtn").prop("disabled", false);
        return;
    }
    let data = {
        date: $("#datepicker").val(),
    ...
    showLoading();
    $.ajax({
        url: `/purchase/make-pallet-barcode`,
        method: 'POST',
        ...
        success: function (result) {
            ...
            if (response == "success") {
                localStorage.removeItem('barcodeListPallet');
                $("#dataTableBody").empty();
                $("#count").text("0");
                $("#totalqty").text("0");
...
function renderTable() {		//테이블그리기
    ...
    if (typeof localStorage !== 'undefined' && localStorage.getItem("barcodeListPallet")) {
        barcodeArray = JSON.parse(localStorage.getItem("barcodeListPallet"));
    } else {
        barcodeArray = [];
    }
    table.empty();
...
function deleteEntry(bar, qty) {		// localstorage에서 특정데이터 삭제
    ...
    Utils.showConfirm(m("confirm.delete.item"), () => {
        let barcodeArray = JSON.parse(localStorage.getItem("barcodeListPallet") || "[]");
        let newArray = barcodeArray.filter(item => item.toString().trim() !== bar.toString().trim());
        localStorage.setItem("barcodeListPallet", JSON.stringify(newArray));
