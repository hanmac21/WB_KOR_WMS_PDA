...
$(document).ready(function () {
    ...
    if (localStorage.getItem("rememberedFactory") === "Saltillo") {
        if (window.WMS.currentModule == 'sales') {
            const title = window.I18N?.['menu.aunde.move.list'] || '';
            if (title) {
    ...
    for (let i = 0; i < list.length; i++) {
        let tbody = `
			<tr>
				...
				<td class = "dataInfo">${(list?.[i]?.FACTORY == null || list?.[i]?.FACTORY === 'null') ? '' : list[i].FACTORY}</td>	
				<td class = "dataInfo">${(list?.[i]?.STORAGE1 == null || list?.[i]?.STORAGE1 === 'null') ? '' : list[i].STORAGE1}</td>	
...
function search() {
    ...
    // 데이터 가져오기
    $.ajax({
        url: "/purchase/search-movement/warehouse-detail",
        method: 'POST',
