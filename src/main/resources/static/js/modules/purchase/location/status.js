...
$(document).ready(function() {
	...
	window.generateStructure = function(rackId, rackData) {
		...
		// 🆕 데이터가 없어도 기본 모듈 수 설정
		//const moduleCount = rackData?.MODULES ?? 9;  // 기본값 9
		const positionCount = Array.isArray(rackData)
            ? rackData.length
            : (rackData?.modules?.reduce((sum, m) => sum + (m.positions?.length || 0), 0) || 0);
           const moduleCount = rackData?.MODULES ?? (positionCount > 0 ? Math.ceil(positionCount / 8) : 1);
		...
		let isValid = false;
		/*if (rackData.factory?.toUpperCase() === "WBTA" && rackData.STORAGE?.toUpperCase() === "MATERIAL"){
			isValid = ["B","D","F","H","J","L","N","P","R"].includes(rackId);			
		}*/
		if (rackData.factory?.toUpperCase() === "WBTA" && rackData.STORAGE?.toUpperCase() === "INBOUND") {
			isValid = ["A","B", "D", "F", "H", "J", "L", "N", "P", "R","T","U","V","W","X","Y"].includes(rackId);
		} else if (rackData.factory?.toUpperCase() === "WBTA" && rackData.STORAGE?.toUpperCase() === "FABRIC") {
			isValid = ["B", "D", "F", "H", "J", "L", "N", "Q", "S", "U", "W", "Y"].includes(rackId);
		}
		
		// Module 생성 - DB 데이터 기반으로 수정
		for (let i = 1; i <= moduleCount; i++) {
		    const num = isValid ? (moduleCount + 1 - i) : i;
			...
			if($("#rack-filter-factoryVal").val() == 'WBTA'){
				...
				['D', 'C', 'B', 'A'].forEach(level => {
					...
					for (let pos = 1; pos <= 2; pos++) {	
					    ...
					    const realPos = isValid ? (3 - pos) : pos;
					    
					    const positionId = `${rackId}-${num}-${level}-${realPos}`;
						console.log("position id :"+positionId)
						// 🆕 **DB 연동 6**: DB에서 해당 포지션의 실제 데이터 가져오기
	                ...
	                `;
						...
						if (status === 'occupied') {
							// DB에서 받은 실제 차량 정보 사용
							html += `<div class="car-info">QTY : ${positionData.qty || 'N/A'}<br>${positionData.itemcode || 'N/A'}<br></div>`;
	...
	// 🔥 **라인 175 함수 시그니처 변경**: positionData 매개변수 추가
//	window.clickPosition = function(location, status, positionData = {}) {
//		const statusText = status === 'occupied' ? '적재됨' : '빈 공간';
//
//		// 모달에 데이터 설정
//		$('#modalLocation').text(location);
//		$('#modalItemCode').text(positionData.itemcode);
//		$('#modalItemName').text(positionData.itemname_mi);
//		$('#modalCarType').text(positionData.carname);
//		$('#modalInboundDate').text(positionData.indate_wms);
//		/*$('#modalQty').text(positionData.qty);*/
//		$('#modalStorageDate').text(positionData.indate);
//		let barcode = positionData.barcode.split(",");
//		if(barcode.length == 5 && positionData.barcode.endsWith("MEX")){		// 파트라벨 바코드
//			$('#modalLot').text("20"+positionData.barcode.split(",")[1]+'-'+positionData.barcode.split(",")[2]);
//		}else if(barcode.length == 1 && barcode[0][0] == "P"){
//			$('#modalLot').text("20"+barcode[0].substring(1,7)+'-'+barcode[0].substring(7,12));
//		}else if(barcode[0][0] == "P" &&positionData.barcode.endsWith("MEX")){
//			$('#modalLot').text("20"+barcode[0].substring(1,7)+'-'+barcode[0].substring(7,12));
//		}
//		//$('#modalStatus').text(statusText);
//		console.log(" DBG 2 -- positionData");
//		console.log(" DBG 2 -- positionData" + positionData.barcode);
//		console.log(positionData);
//		if (status === 'occupied') {
//			// 🔥 **라인 182-186 주석처리**: 임시 랜덤 데이터 제거
//			// const carNumbers = ['CAR-001', 'CAR-002', 'CAR-003', 'CAR-004', 'CAR-005'];
//			// const products = ['HeadRest Assembly', 'DoorLatch Component', 'Safety Belt', 'Airbag Module', 'Seat Frame'];
//			// $('#modalCarNumber').text(carNumbers[Math.floor(Math.random() * carNumbers.length)]);
//			// $('#modalProduct').text(products[Math.floor(Math.random() * products.length)]);
//			// $('#modalQuantity').text(Math.floor(Math.random() * 200) + 50);
//
//			// 🆕 **DB 연동 7**: 실제 DB 데이터 사용
//			$('#modalCarNumber').text(positionData.carInfo?.carNumber || 'N/A');
//			$('#modalProduct').text(positionData.carInfo?.productName || 'N/A');
//			$('#modalQuantity').text(positionData.carInfo?.quantity || '0');
//		} else {
//			// 빈 공간인 경우
//			$('#modalCarNumber').text('N/A');
//			$('#modalProduct').text('N/A');
//			$('#modalQuantity').text('0');
//		}
//
//		// 모달 표시
...
//	}
	...
	window.clickPosition = function(location, status, positionData) {
		...
		const barcode = positionData.barcode || positionData.BARCODE || '';
		$('#modalLocation').text(location);
...
//			console.log(data);
			infoHtml += `
					<div class="modal-data">							
						...
						<div class="modal-info">
                            <div class="info-label">OITEMCODE</div>
                            <div class="info-value" id="modalItemCode">${data.OITEMCODE}</div>
                        </div>
						<div class="modal-info">
						...
						<div class="modal-info">
							...
							<div class="info-value" id="modalInboundDate">${data.LOCDATE}</div>
						</div>
						<div class="modal-info">
	...
	function initFactoryStorageFilter($root) {
	  ...
	  function updatePart2(siteRaw) {
		const site    = norm(siteRaw);
		  ...
		  const storedWarehouse = (localStorage.getItem("rememberedWarehouse") || "").trim();
		  if (storedWarehouse === 'ILLINOIS' && allowed.includes("OUTSIDE")) {
		    initialVal = "OUTSIDE";
		  } 
		...
		rackListData.forEach(function(rack) {
			...
			rackHtml += `
				<div class="rack-item" data-rack="${rack.rackId}" onclick="selectRack('${rack.rackId}')">
					...
					<div class="rack-detail">${rack.totalCapacity}</div>
				</div>
			`;
 ...
 const totalCapacity = rackListData.reduce((sum, rack) => sum + rack.TOTALCAPACITY, 0);
		$('.rack-filter-resultArea').html(`
			${m('filter.title')}<br>
			<span style="font-size: 13pt; opacity: 0.8;">TOTAL : ${totalCapacity}</span>
		`);
	}
	                	...
	                	<div class="accordion-container">
	                		...
	                		<div class="accordion-content active" id="accordion-content">
			                	<div class="rack-filter-area">
					                ...
					                <!--<div class="filter-group">
					                    <label class="filter-label">${m('table.itemCode')}</label>
										<input type="text" id="searchVal-keyword"  onkeypress="if(event.key==='Enter') performSearch()">
					                </div>-->
	...
	// 5. 페이지 로드 시 마지막 선택된 RACK 및 스크롤 위치 복원
	/*window.restoreLastSelectedRack = function() {
	    const lastRackId = localStorage.getItem('lastSelectedRack');
	    const lastRackTime = localStorage.getItem('lastSelectedRackTime');
	    
	    // 24시간 이내의 데이터만 복원
	    const ONE_DAY = 24 * 60 * 60 * 1000;
	    const currentTime = new Date().getTime();
	    
	    if (lastRackId && lastRackTime && (currentTime - parseInt(lastRackTime)) < ONE_DAY) {
	        console.log('🔄 마지막 선택된 RACK 및 스크롤 위치 복원:', lastRackId);
	        
	        // RACK 목록이 로드된 후에 복원
	        setTimeout(() => {
	            const rackElement = $(`[data-rack="${lastRackId}"]`);
	            if (rackElement.length > 0) {
	                // RACK 선택 (스크롤 위치 복원 포함)
	                selectRack(lastRackId);
	                console.log('✅ RACK 및 스크롤 위치 복원 성공:', lastRackId);
	            } else {
	                console.log('❌ 복원할 RACK을 찾을 수 없음:', lastRackId);
	                // 관련 저장 정보 모두 제거
	                clearRackAndScrollStorage(lastRackId);
	            }
	        }, 1000);
	    } else {
	        console.log('🔄 복원할 RACK 정보 없음 또는 만료됨');
	        if (lastRackId) {
	            // 만료된 데이터 제거
	            clearRackAndScrollStorage(lastRackId);
	        }
...
/* --------------------------------------------------------------
 * 📋 DB 연동을 위한 예상 API 응답 데이터 구조
 * -------------------------------------------------------------- */

/*
🔹 **RACK 목록 API 응답 예시** (/api/warehouse/rack/list)
{
	"success": true,
	"data": [
		{
...
🔹 **RACK 상세 API 응답 예시** (/api/warehouse/rack/detail)
{
	"success": true,
	"data": {
		"rackId": "A",
				...
				"positions": [
					{
						...
						"carInfo": {
							"carNumber": "CAR-001",
							"model": "NQ5a",
							"serialNumber": "B201291110101",
							"productName": "HeadRest Assembly",
							"quantity": 280
						}
					},
					{
					...
					// ... 더 많은 포지션 데이터 (총 8개: D-1,D-2,C-1,C-2,B-1,B-2,A-1,A-2)
				]
			}
			// ... 더 많은 모듈 데이터 (총 9개 모듈)
		]
