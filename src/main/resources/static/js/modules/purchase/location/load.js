...
function addEntry() {
    ...
    // 🔹 저장 제한 체크
    if (!savelimitCheck("barcodeListLocation", 500)) {
    ...
    if(barcode.split("_").length === 6 || barcode.split(",")[3] === "WMSUSA" || barcode.split(",")[4] === "WMSUSA"){
        // 박스라벨, 파레트라벨, 파트라벨
        let stored = [];
        if (window.localStorage && localStorage.getItem("barcodeListLocation")) {
            stored = JSON.parse(localStorage.getItem("barcodeListLocation"));
        ...
        if (stored.includes(barcode)) {
            ...
            playSound('complete');
            stored.push(barcode);
            localStorage.setItem("barcodeListLocation", JSON.stringify(stored));
            $("#barcodeInput").val("");
            renderTable();
            hideLoading();
...
function saveLocation0() {
    ...
    const barcodeList = JSON.parse(localStorage.getItem("barcodeListLocation") || "[]");
            ...
            success: function (count) {
                if (count == 0) {
                    ...
                    threeButtonConfirm(
                        ...
                        () => { isSaving = false; }                          // 빈 함수 또는 null
                    );
                    /*Utils.showConfirm('There is a product at that location. Would you like to add it?', () => {
                        existBarcode('add');
                    },
                    () =>{
                        existBarcode('replace');
                    })*/
                }
            },
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (response == "success") {
                    ...
                    localStorage.removeItem("barcodeListLocation");
                    renderTable();
                    /*$("#locationPart1").val("no")
                    $("#locationPart2").val("no")
...
function renderTable() {		//테이블그리기
    ...
    if (typeof localStorage !== 'undefined' && localStorage.getItem("barcodeListLocation")) {
        barcodeArray = JSON.parse(localStorage.getItem("barcodeListLocation"));
    } else {
        barcodeArray = [];
    }
    table.empty();
...
function deleteEntry(bar, qty) {		// localstorage에서 특정데이터 삭제
    ...
    Utils.showConfirm(m("confirm.delete.item"), () => {
        let barcodeArray = JSON.parse(localStorage.getItem("barcodeListLocation") || "[]");
        let newArray = barcodeArray.filter(item => item.toString().trim() !== bar.toString().trim());
        localStorage.setItem("barcodeListLocation", JSON.stringify(newArray));
