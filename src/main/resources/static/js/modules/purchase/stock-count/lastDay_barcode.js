...
function addEntry() {			// 로컬스토리지 저장
    ...
    // 🔹 저장 제한 체크
    if (!savelimitCheck("barcodeListRealStockLastDayBarcode", 500)) {
    ...
    if (window.localStorage && localStorage.getItem("barcodeListRealStockLastDayBarcode")) {
        stored = JSON.parse(localStorage.getItem("barcodeListRealStockLastDayBarcode"));
    } else {
        stored = [];
    }
    ...
    if (stored.includes(barcode)) {
        ...
        stored.push(barcode);
        localStorage.setItem("barcodeListRealStockLastDayBarcode", JSON.stringify(stored));
        $("#barcodeInput").val("");
        renderTable();
        hideLoading();
...
function saveBarcode() {	// 파트바코드가 팔레트로 묶여진건지 확인
    const barcodeList = JSON.parse(localStorage.getItem("barcodeListRealStockLastDayBarcode") || "[]");
...
function saveBarcode2(status, list) {					// 전체전송
    ...
    Utils.showConfirm(m("confirm.send.all"), () => {
        ...
        $.ajax({
            url: `/purchase/insertRealStockLastDay`,
            method: 'POST',
            contentType: "application/json",
            data: JSON.stringify(data),
            success: function (result) {
                ...
                console.log("response : " + response)
                if (response === "success") {
                    localStorage.removeItem('barcodeListRealStockLastDayBarcode');
                    $("#dataTableBody").empty();
                    $("#count").text("0");
                    $("#totalqty").text("0");
                    playSound("complete");
...
function renderTable() {		//테이블그리기
    ...
    if (typeof localStorage !== 'undefined' && localStorage.getItem("barcodeListRealStockLastDayBarcode")) {
        barcodeArray = JSON.parse(localStorage.getItem("barcodeListRealStockLastDayBarcode"));
    } else {
        barcodeArray = [];
    }
    table.empty();
    for (let i = 0; i < barcodeArray.length; i++) {
        ...
        if (barcodeOneArr.length == 4) {
            ...
            console.log(Number(barcodeOneArr[3]));
...
function deleteEntry(bar, qty) {		// localstorage에서 특정데이터 삭제
    ...
    Utils.showConfirm(m("confirm.delete.item"), () => {
        let barcodeArray = JSON.parse(localStorage.getItem("barcodeListRealStockLastDayBarcode") || "[]");
        let newArray = barcodeArray.filter(item => item.toString().trim() !== bar.toString().trim());
        localStorage.setItem("barcodeListRealStockLastDayBarcode", JSON.stringify(newArray));
