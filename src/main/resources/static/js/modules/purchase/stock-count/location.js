...
function addEntry() {
    ...
    // 🔹 저장 제한 체크
    if (!savelimitCheck("barcodeListLocationCount", 500)) {
    ...
    if ((barcode.split(",").length === 5 && (barcode.split(",")[4] === "SCMMEX" || barcode.split(",")[4] === "WMSMEX" || barcode.split(",")[4] === "WMSUSA"))
        ...
        let stored = [];
        let saveItemcode = "";
        if (window.localStorage && localStorage.getItem("barcodeListLocationCount")) {
            stored = JSON.parse(localStorage.getItem("barcodeListLocationCount"));
        ...
        if (stored.includes(barcode)) {
            ...
            stored.push(barcode);
            localStorage.setItem("barcodeListLocationCount", JSON.stringify(stored));
            $("#barcodeInput").val("");
            renderTable();
            hideLoading();
...
function saveLocation0() {
    ...
    isSaving = true;
    const barcodeList = JSON.parse(localStorage.getItem("barcodeListLocationCount") || "[]");
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (response == "success") {
                    ...
                    localStorage.removeItem("barcodeListLocationCount");
                    renderTable();
...
function renderTable() {		//테이블그리기
    ...
    if (typeof localStorage !== 'undefined' && localStorage.getItem("barcodeListLocationCount")) {
        barcodeArray = JSON.parse(localStorage.getItem("barcodeListLocationCount"));
    } else {
        barcodeArray = [];
    }
    table.empty();
...
function deleteEntry(bar, qty) {		// localstorage에서 특정데이터 삭제
    ...
    Utils.showConfirm(m("confirm.delete.clear"), () => {
        let barcodeArray = JSON.parse(localStorage.getItem("barcodeListLocationCount") || "[]");
        let newArray = barcodeArray.filter(item => item.toString().trim() !== bar.toString().trim());
        localStorage.setItem("barcodeListLocationCount", JSON.stringify(newArray));
