...
function search() {
    ...
    let data = {
        factory: localStorage.getItem('rememberedFactory'), storage: $(".storage-select").val(), date: date
    }
    // 로딩창
    showLoading();
    ...
    // 데이터 가져오기
    $.ajax({
        url: "/purchase/search-exception/output-detail",
        method: 'POST',
...
$(document).on("click", ".dataInfo", function () {
    ...
    $.ajax({
        url: "/purchase/getItemInfo", type: "GET", data: {itemcode}, dataType: "json", success: function (result) {
            console.log(result);
