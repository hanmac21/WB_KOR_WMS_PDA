...
$(document).ready(function () {
    ...
    $(".storage-select").val(_defaultStorage).trigger("change");
...
function addEntry() {			// 로컬스토리지 저장
    ...
    // 🔹 저장 제한 체크
    if (!savelimitCheck("barcodeListOutputReturn", 500)) {
    ...
    if (isValidBarcode) {
        ...
        if (window.localStorage && localStorage.getItem("barcodeListOutputReturn")) {
            stored = JSON.parse(localStorage.getItem("barcodeListOutputReturn"));
        } else {
            stored = [];
        }
        ...
        if (stored.includes(barcode)) {
            ...
            stored.push(barcode);
            localStorage.setItem("barcodeListOutputReturn", JSON.stringify(stored));
            $("#barcodeInput").val("");
            renderTable();
            hideLoading();
...
function saveBarcode() {					// 전체전송
    ...
    const barcodeList = JSON.parse(localStorage.getItem("barcodeListOutputReturn") || "[]");
    ...
    Utils.showConfirm(m("confirm.send.all"), () => {
            showLoading();
            $.ajax({
                url: "/purchase/updateExceptionCheckStatus",
                type: "POST",
                ...
                success: function (result) {
                    ...
                    if (response === "success") {
                        localStorage.removeItem('barcodeListOutputReturn');
                        $("#dataTableBody").empty();
                        $("#count").text("0");
                        $("#totalqty").text("0");
...
function renderTable() {		//테이블그리기
    ...
    if (typeof localStorage !== 'undefined' && localStorage.getItem("barcodeListOutputReturn")) {
        barcodeArray = JSON.parse(localStorage.getItem("barcodeListOutputReturn"));
    } else {
        barcodeArray = [];
    }
    table.empty();
...
function deleteEntry(bar, qty) {		// localstorage에서 특정데이터 삭제
    ...
    Utils.showConfirm(m("confirm.delete.item"), () => {
        let barcodeArray = JSON.parse(localStorage.getItem("barcodeListOutputReturn") || "[]");
        let newArray = barcodeArray.filter(item => item.toString().trim() !== bar.toString().trim());
        localStorage.setItem("barcodeListOutputReturn", JSON.stringify(newArray));
