$(document).ready(function () {
    ...
    renderTable();
    const savedInvoice = localStorage.getItem("invoiceDataTransysGAOut");
    if (savedInvoice) {
...
function addEntry() {			// 로컬스토리지 저장
    ...
    if (barcode.includes(":") && barcode.startsWith("[)>")) {
        // 인보이스 바코드 (CY[ 포함)
        if (barcode.includes(":CY[")) {
            if (localStorage.getItem("invoiceDataTransysGAOut")) {
                playSound('error');
                $("#barcodeInput").val("");
                hideLoading();
            ...
            if (info) {
                $('#invoiceManifestNo').text(info.manifestNo);
                localStorage.setItem("invoiceDataTransysGAOut", JSON.stringify(info));
                renderTable();
                playSound('complete');
            } else {
                playSound('error');
        ...
        // 인보이스 데이터 있으면 파트 바코드 매칭 모드
        const invoiceRaw = localStorage.getItem("invoiceDataTransysGAOut");
        if (!invoiceRaw) {      // 인보이스 바코드 스캔안되어 있으면 인보이스 스캔하라고 메시지
            playSound('error');
        ...
        const scannedList = JSON.parse(localStorage.getItem("partBarcodeListTransysGAOut") || "[]");
        if (scannedList.includes(barcode)) {
            ...
            if (item.scannedQty >= item.qty) {
                ...
                localStorage.setItem("partBarcodeListTransysGAOut", JSON.stringify(scannedList));
                localStorage.setItem("invoiceDataTransysGAOut", JSON.stringify(invoice));
                renderTable();
...
function saveBarcode() {					// 전체전송
    ...
    const barcodeList = JSON.parse(localStorage.getItem("partBarcodeListTransysGAOut") || "[]");
    ...
    Utils.showConfirm(m("confirm.send.all"), () => {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (response === "success") {
                        localStorage.removeItem('partBarcodeListTransysGAOut');
                        localStorage.removeItem('invoiceDataTransysGAOut');
                        $("#dataTableBody").empty();
                        $("#count").text("0");
...
function parsePartBarcode(barcode) {
    // [)>*06:3SBWDKVDI00211:P89900XG100WK:5Q15:D:...:*EOT
    const parts = barcode.split(":");
    if (parts.length < 4) return null;
    const itemCodePart = parts[2];  // P89900XG100WK
    const qtyPart = parts[3];       // 5Q15
    if (!itemCodePart.startsWith("P")) return null;
    const itemCode = itemCodePart.substring(1);
    const qtyMatch = qtyPart.match(/(\d+)$/);
    if (!qtyMatch) return null;
    return { itemCode, qty: parseInt(qtyMatch[1], 10) };
}

// 트랜시스 인보이스 바코드 파싱
function parseInvoiceBarcode(barcode) {
    // Format: [)>*09:MANIFEST_NO:PT4201:CY[ITEM1=QTY1,ITEM2=QTY2,...]:*EOT
    const manifestMatch = barcode.match(/\*[^:]+:([^:]+):/);
    if (!manifestMatch) return null;
    const manifestNo = manifestMatch[1];

    // 인보이스 바코드와 트랜시스파트바코드를 구분
    const cyMatch = barcode.match(/CY\[([^\]]+)\]/);
    if (!cyMatch) return null;
    const itemStrs = cyMatch[1].split(',');

    let totalQty = 0;
    const items = [];
    itemStrs.forEach(itemStr => {
        const eqIdx = itemStr.lastIndexOf('=');
        if (eqIdx !== -1) {
            const itemCode = itemStr.substring(0, eqIdx);
            const qty = parseInt(itemStr.substring(eqIdx + 1), 10) || 0;
            totalQty += qty;
...
function openBarcodeModal(itemCode) {
    const allBarcodes = JSON.parse(localStorage.getItem("partBarcodeListTransysGAOut") || "[]");
    const matched = allBarcodes.filter(bc => {
        const parsed = parsePartBarcode(bc);
...
function deleteBarcodeFromItem(barcode, itemCode) {
    Utils.showConfirm(m("confirm.delete.item"), () => {
        ...
        let allBarcodes = JSON.parse(localStorage.getItem("partBarcodeListTransysGAOut") || "[]");
        allBarcodes = allBarcodes.filter(bc => bc !== barcode);
        localStorage.setItem("partBarcodeListTransysGAOut", JSON.stringify(allBarcodes));
