$(document).ready(function () {
    ...
    renderTable();
    const savedInvoice = localStorage.getItem("invoiceDataTransysILOut");
    if (savedInvoice) {
...
function addEntry() {			// 로컬스토리지 저장
    ...
    if (barcode.includes(":") && barcode.startsWith("[)>")) {
        ...
        if (barcode.includes(":CY[")) {
            if (localStorage.getItem("invoiceDataTransysILOut")) {
                playSound('error');
                $("#barcodeInput").val("");
                hideLoading();
            ...
            if (info) {
                $('#invoiceManifestNo').text(info.manifestNo);
                localStorage.setItem("invoiceDataTransysILOut", JSON.stringify(info));
                renderTable();
                playSound('complete');
            } else {
                playSound('error');
        ...
        const invoiceRaw = localStorage.getItem("invoiceDataTransysILOut");
        ...
        const scannedList = JSON.parse(localStorage.getItem("partBarcodeListTransysILOut") || "[]");
        if (scannedList.includes(barcode)) {
            ...
            if (item.scannedQty >= item.qty) {
                ...
                localStorage.setItem("partBarcodeListTransysILOut", JSON.stringify(scannedList));
                localStorage.setItem("invoiceDataTransysILOut", JSON.stringify(invoice));
                renderTable();
...
function saveBarcode() {					// 전체전송
    ...
    const barcodeList = JSON.parse(localStorage.getItem("partBarcodeListTransysILOut") || "[]");
    ...
    Utils.showConfirm(m("confirm.send.all"), () => {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (response === "success") {
                        localStorage.removeItem('partBarcodeListTransysILOut');
                        localStorage.removeItem('invoiceDataTransysILOut');
                        $("#dataTableBody").empty();
                        $("#count").text("0");
...
function openBarcodeModal(itemCode) {
    const allBarcodes = JSON.parse(localStorage.getItem("partBarcodeListTransysILOut") || "[]");
...
function deleteBarcodeFromItem(barcode, itemCode) {
    Utils.showConfirm(m("confirm.delete.item"), () => {
        ...
        let allBarcodes = JSON.parse(localStorage.getItem("partBarcodeListTransysILOut") || "[]");
        allBarcodes = allBarcodes.filter(bc => bc !== barcode);
        localStorage.setItem("partBarcodeListTransysILOut", JSON.stringify(allBarcodes));
