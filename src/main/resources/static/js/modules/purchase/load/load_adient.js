$(document).ready(function () {
    ...
    renderTable();
    const savedInvoice = localStorage.getItem("invoiceDataAdientOut");
    if (savedInvoice) {
...
function addEntry() {			// 로컬스토리지 저장
    ...
    if ((barcode.includes("|") || barcode.includes("ADNT")) && barcode.startsWith("[)>") ) {   // 에디언트 인보이스 바코드만
        // 인보이스 바코드 ($$ 포함)
        if (barcode.includes("$$")) {
            if (localStorage.getItem("invoiceDataAdientOut")) {
                playSound('error');
                $("#barcodeInput").val("");
                hideLoading();
            ...
            if (info) {
                $('#invoiceManifestNo').text(info.manifestNo);
                localStorage.setItem("invoiceDataAdientOut", JSON.stringify(info));
                renderTable();
                playSound('complete');
            } else {
                playSound('error');
        ...
        const invoiceRaw = localStorage.getItem("invoiceDataAdientOut");
        ...
        const scannedList = JSON.parse(localStorage.getItem("partBarcodeListAdientOut") || "[]");
        if (scannedList.includes(barcode)) {
            ...
            if (item.scannedQty >= item.qty) {
                ...
                localStorage.setItem("partBarcodeListAdientOut", JSON.stringify(scannedList));
                localStorage.setItem("invoiceDataAdientOut", JSON.stringify(invoice));
                renderTable();
...
function saveBarcode() {					// 전체전송
    ...
    const barcodeList = JSON.parse(localStorage.getItem("partBarcodeListAdientOut") || "[]");
    ...
    Utils.showConfirm(m("confirm.send.all"), () => {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (response === "success") {
                        localStorage.removeItem('partBarcodeListAdientOut');
                        localStorage.removeItem('invoiceDataAdientOut');
                        $("#dataTableBody").empty();
                        $("#count").text("0");
...
// 에디언트 파트바코드 파싱
// Format: [)>|06|P6718092-DQK|Q3600|S7218|V354392||
function parsePartBarcode(barcode) {
    const parts = barcode.split("|");
    let itemCode = null, qty = null, serial = null;
    for (const part of parts) {
        if (itemCode === null && part.startsWith("P")) itemCode = part.substring(1);
        else if (qty === null && part.startsWith("Q")) qty = parseInt(part.substring(1), 10);
        else if (serial === null && part.startsWith("S")) serial = part.substring(1);
    }
    if (!itemCode || isNaN(qty)) return null;
    return { itemCode, qty, serial };
}

// 에디언트 인보이스 바코드 파싱
// Format: [)>$$ADNT$$WBTA20260420_#1$$L002231880NCPAA:1$$L002556779NCPAB:48$$...
...
function openBarcodeModal(itemCode) {
    const allBarcodes = JSON.parse(localStorage.getItem("partBarcodeListAdientOut") || "[]");
...
function deleteBarcodeFromItem(barcode, itemCode) {
    Utils.showConfirm(m("confirm.delete.item"), () => {
        ...
        let allBarcodes = JSON.parse(localStorage.getItem("partBarcodeListAdientOut") || "[]");
        allBarcodes = allBarcodes.filter(bc => bc !== barcode);
        localStorage.setItem("partBarcodeListAdientOut", JSON.stringify(allBarcodes));
