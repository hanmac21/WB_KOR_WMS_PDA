...
function addEntry() {			// 로컬스토리지 저장
    ...
    const shipTo = $(".shipto-select").val();
    const isValidBarcode = shipTo === "ADNT"
        ? barcode.startsWith("[)>")
        : barcode.split(",")[4] === "WMSUSA" || barcode.split(",")[3] === "WMSUSA" ||  barcode.split("_").length === 6;

    if (isValidBarcode && !barcode.includes(":")) {
        // 파트라벨 바코드인지 확인
        let stored = [];
