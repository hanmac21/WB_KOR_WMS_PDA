$(document).ready(function () {
    ...
    renderTable();
    const savedInvoice = localStorage.getItem("invoiceDataTransysALOut");
    if (savedInvoice) {
...
function addEntry() {			// 로컬스토리지 저장
    ...
    if (barcode.includes(":") && barcode.startsWith("[)>")) {
        ...
        if (barcode.includes(":CY[")) {
            if (localStorage.getItem("invoiceDataTransysALOut")) {
                playSound('error');
                $("#barcodeInput").val("");
                hideLoading();
            ...
            if (info) {
                $('#invoiceManifestNo').text(info.manifestNo);
                localStorage.setItem("invoiceDataTransysALOut", JSON.stringify(info));
                renderTable();
                playSound('complete');
            } else {
                playSound('error');
        ...
        const invoiceRaw = localStorage.getItem("invoiceDataTransysALOut");
        ...
        const scannedList = JSON.parse(localStorage.getItem("partBarcodeListTransysALOut") || "[]");
        if (scannedList.includes(barcode)) {
            ...
            if (item.scannedQty >= item.qty) {
                ...
                localStorage.setItem("partBarcodeListTransysALOut", JSON.stringify(scannedList));
                localStorage.setItem("invoiceDataTransysALOut", JSON.stringify(invoice));
                renderTable();
...
function saveBarcode() {					// 전체전송
    ...
    const barcodeList = JSON.parse(localStorage.getItem("partBarcodeListTransysALOut") || "[]");
    ...
    Utils.showConfirm(m("confirm.send.all"), () => {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (response === "success") {
                        localStorage.removeItem('partBarcodeListTransysALOut');
                        localStorage.removeItem('invoiceDataTransysALOut');
                        $("#dataTableBody").empty();
                        $("#count").text("0");
...
function openBarcodeModal(itemCode) {
    const allBarcodes = JSON.parse(localStorage.getItem("partBarcodeListTransysALOut") || "[]");
...
function deleteBarcodeFromItem(barcode, itemCode) {
    Utils.showConfirm(m("confirm.delete.item"), () => {
        ...
        let allBarcodes = JSON.parse(localStorage.getItem("partBarcodeListTransysALOut") || "[]");
        allBarcodes = allBarcodes.filter(bc => bc !== barcode);
        localStorage.setItem("partBarcodeListTransysALOut", JSON.stringify(allBarcodes));
