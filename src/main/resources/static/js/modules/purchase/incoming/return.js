...
$(document).ready(function () {
    ...
    $(".storage-select").val("PRODUCT").trigger("change");
    renderTable();
...
function addEntry() {			// 로컬스토리지 저장
    ...
    // 🔹 저장 제한 체크
    if (!savelimitCheck("barcodeListInReturn", 500)) {
    ...
    if(barcode.split("_").length === 6 || barcode.split(",")[3] === "WMSUSA" || barcode.split(",")[4] === "WMSUSA"){
        ...
        if (window.localStorage && localStorage.getItem("barcodeListInReturn")) {
            stored = JSON.parse(localStorage.getItem("barcodeListInReturn"));
        } else {
            stored = [];
        }
        ...
        if (stored.includes(barcode)) {
            ...
            stored.push(barcode);
            localStorage.setItem("barcodeListInReturn", JSON.stringify(stored));
            $("#barcodeInput").val("");
            renderTable();
            hideLoading();
...
function saveBarcode() {					// 전체전송
    ...
    // 전체전송
    const barcodeList = JSON.parse(localStorage.getItem("barcodeListInReturn") || "[]");
            ...
            try {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        if (response == "success") {
                            localStorage.removeItem('barcodeListInReturn');
                            $("#dataTableBody").empty();
                            $("#count").text("0");
                            $("#totalqty").text("0");
...
function renderTable() {		//테이블그리기
    ...
    if (typeof localStorage !== 'undefined' && localStorage.getItem("barcodeListInReturn")) {
        barcodeArray = JSON.parse(localStorage.getItem("barcodeListInReturn"));
    } else {
        barcodeArray = [];
    }
    table.empty();
...
function deleteEntry(bar, qty) {		// localstorage에서 특정데이터 삭제
    ...
    Utils.showConfirm(m("confirm.delete.item"), () => {
        let barcodeArray = JSON.parse(localStorage.getItem("barcodeListInReturn") || "[]");
        let newArray = barcodeArray.filter(item => item.toString().trim() !== bar.toString().trim());
        localStorage.setItem("barcodeListInReturn", JSON.stringify(newArray));
