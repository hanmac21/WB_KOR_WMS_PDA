...
function renderTable(list) {		//테이블그리기
	...
	for(let i = 0; i < list.length; i++){
		let tbody = `
			<tr>
				<td class = "dataInfo">${(list?.[i]?.OITEMCODE == null || list?.[i]?.OITEMCODE === 'null') ? '' : list[i].OITEMCODE}</td>
				<td class = "dataInfo">${(list?.[i]?.QTY == null || list?.[i]?.QTY === 'null') ? '' : list[i].QTY}</td>
				<td class = "dataInfo">${(list?.[i]?.SOURCE == null || list?.[i]?.SOURCE === 'null') ? '' : list[i].SOURCE}</td>	
...
function search(){
	...
	const cust = $(".customer-select").val();
	
	const custcode = (cust ?? '').trim().split("_")[0];
	const custname = (cust ?? '').trim().split("_")[1];
	...
	// 데이터 가져오기
	$.ajax({
		url: "/purchase/search-incoming-detail",		
        method: 'POST',
