...
function addEntry() {		// 영업이송에서 스캔됐을때 로컬스토리지에 안넣고 그냥 화면에 보여줌
	...
	if ((barcode.split(",").length == 5 && (barcode.split(",")[4] == "SCMMEX" || barcode.split(",")[4] == "WMSMEX")) ) {
		$("#scanBarcode").val(barcode)
		if($("#scanBarcode2").val().trim()==''){
			console.log("barcode1 scan")
			playSound('complete')
		}
	} else {
	...
	// 🔹 저장 제한 체크
	if (!savelimitCheck("barcodeListSalesTransfer", 500)) {
	...
	if ((barcode.split(",").length == 5 && (barcode.split(",")[4] == "SCMMEX" || barcode.split(",")[4] == "WMSMEX")) || (barcode[0][0] == "P" && barcode.endsWith("MEX") || barcode.split(",").length == 1 && barcode[0][0] == "P")) {
		...
		if (window.localStorage && localStorage.getItem("barcodeListSalesTransfer")) {
			stored = JSON.parse(localStorage.getItem("barcodeListSalesTransfer"));
		} else {
			stored = [];
		}
		...
		if (stored.includes(barcode)) {
			...
			audio.play();
			stored.push(barcode);
			localStorage.setItem("barcodeListSalesTransfer", JSON.stringify(stored));
			$("#barcodeInput").val("");
			renderTable();
			hideLoading();
