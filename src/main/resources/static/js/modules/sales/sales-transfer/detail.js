	...
	for(let i = 0; i < list.length; i++){
		...
		let tbody = `
			<tr>
				...
				<td><button class="delete-btn" onclick="deleteEntry('${list?.[i]?.BARCODE}','${list?.[i]?.MES_KEY}' ,'${list?.[i]?.KEY_EXIST}')">${m('btn.delete')}</button></td>
            </tr>
		`;
...
function deleteEntry(barcode,meskey, keyExist){
	showLoading();
	// meskey 유효성 체크	
    if (!meskey || (typeof keyExist === "string" && keyExist.trim() === "" || keyExist =='undefined' || keyExist == "N")) {
	...
	Utils.showConfirm(m("confirm.delete.item"), () => {
		$.ajax({
			url: "/sales/search-transfer/detail/del",		
	        method: 'POST',
			data: {
				barcode:barcode,
				meskey:meskey,
...
function search(){
	...
	showLoading();
	$.ajax({
		url: "/sales/sales-transfer/detail",		
        method: 'POST',
