...
$(document).ready(function () {
    ...
    // 0) 로컬스토리지의 공장 값(케이스 변경 없이) 읽기
    const storedFactory = (localStorage.getItem("rememberedFactory") || "").trim();
    const rawWarehouse = localStorage.getItem("rememberedWarehouse");
    const storedWarehouse = (rawWarehouse && rawWarehouse !== "null" ? rawWarehouse : "").trim();

    // 1) 공장별 허용 규칙 (기존 케이스 그대로)
    const STORAGE_RULES = {
        'WBTA': ['INBOUND', 'PRODUCT', 'OUTSIDE'],
    };

    // 2) .storage-select 갱신
//	 function updateStorageByFactory(factory) {
//		 const allowed = STORAGE_RULES[factory] || [];
//		 $(".storage-select").each(function () {
//			 const select = $(this);
//			 select.empty();
//			 allowed.forEach(opt => select.append(new Option(opt, opt)));
//			 if (allowed.length > 0) select.val(allowed[0]).trigger('change');
//		 });
//	 }

    // 3) locationPart1(공장) DOM 반영 (케이스 그대로 매칭)
    const $part1 = $("#locationPart1");
    if ($part1.length) {
        // 옵션 목록에서 동일 값(케이스 그대로) 찾기
        const hasOption = Array.from($part1[0].options).some(o => (o.value || "").trim() === storedFactory);
        if (storedFactory && hasOption) {
            $part1.val(storedFactory).trigger('change'); // change 트리거로 의존 로직도 깨우기
        }
    }

    // 4) .storage-select 먼저 재구성
    const effectiveFactory = ($part1.val() || "").trim();
    updateStorageByFactory(effectiveFactory);

    // 5) 모듈 복원 → locationPart2 값(있을 때만) 적용
    ...
    if (storedModule) {
        ...
        // 약간의 지연 후 확인
        setTimeout(() => {
            const $part2 = $("#locationPart2");
            const canSet = (val) => !!$part2.length && Array.from($part2[0].options).some(o => (o.value || "").trim() === val);
            if (storedWarehouse === 'ILLINOIS' && canSet("OUTSIDE")) {
                $part2.val("OUTSIDE").trigger('change');
                console.log("ILLINOIS 창고 선택됨 - 기본 OUTSIDE 적용");
            } else if (window.WMS.currentModule === 'purchase' && canSet("INBOUND")) {
                $part2.val("INBOUND").trigger('change');
                // // 적재, 재고실사 적재일때만 조건이 필요함
                // if (window.location.href.includes('/location/load')
                // 		|| window.location.href.includes('/stock-count/location')
                // 		|| window.location.href.includes('/stock-count/barcode')
                // 		|| window.location.href.includes('/stock-count/lastDay_location')
                // 		|| window.location.href.includes('/stock-count/lastDay_barcode')){
                // 	$part2.val('').trigger('change');
                // }
            } else if (window.WMS.currentModule === 'sales' && canSet("PRODUCT")) {
                $part2.val("PRODUCT").trigger('change');
                console.log("복원된 모듈:", window.WMS.currentModule);
            }
        }, 0);
    } else {
    ...
    // OUT/IN select 갱신 함수
    function updateStorageByFactory(factory) {
        let allowed = STORAGE_RULES[factory] || [];
        ...
        $(".storage-select").each(function () {
            ...
            // 기본 값 처리
            let defaultVal = allowed[0];
            if (storedWarehouse === 'ILLINOIS' && allowed.includes("OUTSIDE")) {
                // ILLINOIS 창고 선택 시 OUTSIDE를 최우선 기본값으로
                defaultVal = "OUTSIDE";
            } else if (window.WMS?.currentModule === "purchase" && allowed.includes("INBOUND")) {
                defaultVal = "INBOUND";
            } else if (window.WMS?.currentModule === "sales" && allowed.includes("PRODUCT")) {
                defaultVal = "PRODUCT";
            }
    ...
    // 실행
    let factory = (localStorage.getItem("rememberedFactory") || "").trim();
    if (!STORAGE_RULES[factory]) factory = "Saltillo"; // 기본값 Saltillo

    updateStorageByFactory(factory);

    hideLoading();
});

// ===== 공장에 따른 의존 셀렉트 재구성(케이스 유지) =====
document.addEventListener('DOMContentLoaded', () => {
    const part1 = document.getElementById('locationPart1');
    const part2 = document.getElementById('locationPart2');
    const part5 = document.getElementById('locationPart5');
    const part6 = document.getElementById('locationPart6');

    if (!part1 || !part2) return;

    // 스냅샷
    const template = Array.from(part2.options).map(o => ({value: o.value, label: o.text}));
    const tpl5 = part5 ? Array.from(part5.options).map(o => ({value: o.value, label: o.text})) : null;
    const tpl6 = part6 ? Array.from(part6.options).map(o => ({value: o.value, label: o.text})) : null;
    ...
    // 기본값: 정확한 키가 없을 수도 있으니, 정확 키 우선 → 없으면 대소문자 무시 대체
    const defaults = {
        part2: {'WBTA': 'MATERIAL'},
        part5: {'WBTA': ''},
        part6: {'WBTA': ''},
    ...
    function getDefault(map, site) {
        // 1순위: 정확 키
        if (map && Object.prototype.hasOwnProperty.call(map, site)) return map[site];
        // 2순위: 대소문자 무시 매칭
        const hit = Object.keys(map || {}).find(k => k.toLowerCase() === site.toLowerCase());
        return hit ? map[hit] : undefined;
    }

    function rebuild(selectEl, tpl, allowed, prefer) {
        if (!selectEl || !tpl) return;
        selectEl.innerHTML = '';
        tpl.forEach(opt => {
            if (!allowed.length || allowed.includes(opt.value)) {
                selectEl.add(new Option(opt.label, opt.value));
            }
        });
        if (prefer && allowed.includes(prefer)) {
            selectEl.value = prefer;
    ...
    function updatePart2(siteRaw) {
        ...
        const allowed = (rules.part2 && rules.part2[site]) || [];
        const prefer = getDefault(defaults.part2, site);
        rebuild(part2, template, allowed, prefer);
    }

    function updatePart5(siteRaw) {
        if (!part5 || !tpl5) return;
        const site = norm(siteRaw);
        const allowed = (rules.part5 && rules.part5[site]) || [];
        const prefer = getDefault(defaults.part5, site);
        rebuild(part5, tpl5, allowed, prefer);
    }

    function updatePart6(siteRaw) {
        if (!part6 || !tpl6) return;
        const site = norm(siteRaw);
        const allowed = (rules.part6 && rules.part6[site]) || [];
        const prefer = getDefault(defaults.part6, site);
        rebuild(part6, tpl6, allowed, prefer);
    }

    // A) 페이지 최초: 로컬스토리지 공장 우선 적용 (케이스 유지)
    const storedFactory = norm(localStorage.getItem("rememberedFactory"));
    const factoryToUse =
        storedFactory && (rules.part2[storedFactory] || rules.part5?.[storedFactory] || rules.part6?.[storedFactory])
            ? storedFactory
            : norm(part1.value); // 로컬스토리지에 없거나 규칙에 없으면 현재 값 유지

    // part1 값 동기화(옵션에 있을 때만)
    const hasPart1 = Array.from(part1.options).some(o => norm(o.value) === factoryToUse);
    if (hasPart1) part1.value = factoryToUse;

    // B) 의존 셀렉트 초기 재구성
    updatePart2(part1.value);
    updatePart5(part1.value);
    updatePart6(part1.value);
